...
_.forEach(commands, function (commandName) {
  Commander.prototype[commandName] = generateFunction(commandName, null);
  Commander.prototype[commandName + 'String'] = generateFunction(commandName, 'utf8');
});

Commander.prototype.call = generateFunction(null);
Commander.prototype.callString = generateFunction('utf8');
...
Commander.prototype.defineCommand = function (name, definition) {
  ...
  this[name] = generateScriptingFunction(script, null);
  this[name + 'String'] = generateScriptingFunction(script, 'utf8');
};
...
function generateFunction (_commandName, _encoding) {
  ...
  return function () {
    ...
    if (callback && this.options.commandTimeout) {
      var originalCallback = _.once(callback);
      ...
      timeoutHandle = setTimeout(function () {
        var err = new Error('Redis timeout');
        if (options.errorStack) {
          err = utils.optimizeErrorStack(err, options.errorStack, __dirname);
        }
        callback(err);
