...
var BufferResultsRedis = require('./bufferResultsRedis');
var HealthChecker = require('health-checker');
 ...
 * @param {boolean} [options.returnBuffers=false] - Return buffers by default.
 * This option maintains compatibility with node_redis.
 * The string variants are still available at `xyzString` (e.g. `getString`).
 * @param {Object} [options.healthCheck=null] - Options for the health-checker. Keep `null` if you do
   not wish to enable the health checker.
 * @param {boolean} [options.healthCheck.forceReconnect=false] - On a health check failure,
   disconnect and reconnect to the redis server (useful when health check failure might mean that
   a DNS resolution has to be done again for the host).
...
function Redis() {
  ...
  if (this.options.returnBuffers && !(this instanceof BufferResultsRedis)) {
    return new BufferResultsRedis(arguments[0], arguments[1], arguments[2]);
  }
  ...
  if (this.options.healthCheck) {
    var hcOptions = _.cloneDeep(this.options.healthCheck);
    var self = this;
    hcOptions.check = function () {
      return self.ping();
    };
    this.healthCheck = new HealthChecker(hcOptions).start();
    this.healthCheck.on('sick', function (reason) {
      self.silentEmit('error', new Error('Redis did not reply to PING in time.'));
      if (self.options.healthCheck.forceReconnect) {
...
BufferResultsRedis = BufferResultsRedis(Redis);
...
/**
 * Get connectivity (and health) status of Redis.
 */
Redis.prototype.connected = function () {
  if (this.status !== 'ready') {
    return false;
  } else if (this.healthCheck && !this.healthCheck.isHealthy()) {
    return false;
  } else {
    return true;
  }
...
Redis.prototype._readyCheck = function (callback) {
  ...
  this.info(function (err, res) {
    ...
    if (!info.loading || info.loading === '0') {
      ...
      var retryTime = Math.min(info.loading_eta_seconds || 1, 30) * 1000;
...
Redis.prototype.sendCommand = function (command, stream) {
  ...
  var healthyHealthCheck = _.includes(['info', 'select', 'ping'], command.name) ||
      !this.healthCheck || this.healthCheck.isHealthy();

  if ((!writable || !healthyHealthCheck) && !this.options.enableOfflineQueue) {
